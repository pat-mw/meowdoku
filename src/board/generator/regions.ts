...
/**
 * Weight multiplier for a pocket candidate by how many fresh rows and columns it
 * would reach into (0, 1 or 2). Blocky pockets are penalised for spreading and
 * so stay square; snaking ones are rewarded, which is what pulls them into a
 * line. Cohesion cannot make this distinction on a region of four cells, because
 * almost every candidate there has a cohesion of one.
 */
const POCKET_LINE_REACH: Record<RegionShape, readonly number[]> = {
  blocky: [24, 4, 1],
  mixed: [1, 1, 1],
  irregular: [1, 3, 4],
  snaking: [1, 8, 20],
 ...
 *
 * The share rises with board size, and both ends of that ladder are forced.
 * Visually the basin should stay close to the real game's reference level, whose
 * largest region is about a third of the board — a board that is two thirds one
 * colour reads as a mistake rather than a puzzle. But constraint has to scale
 * with the search space, which grows explosively: a 15x15 board has orders of
 * magnitude more legal cat placements to rule out than an 11x11, and at a
 * reference-like share no 15x15 board was uniquely solvable at all. So small
 * boards get the handsome partition and the largest boards buy solvability with
 * a bigger basin — where fifteen scattered pockets still give the eye plenty to
 * read.
 */
const basinShareFor = (size: number): number => {
  if (size <= 6) return 360
  if (size <= 8) return 390
  if (size <= 10) return 420
  if (size <= 12) return 460
  if (size === 13) return 580
  if (size === 14) return 700
  return 790
}

const BASIN_SHARE_JITTER = 60
...
const drawTargets = (
  ...
  const share = basinShareFor(size) + rng.nextRange(-BASIN_SHARE_JITTER, BASIN_SHARE_JITTER)
...
const pickCell = (
  ...
  for (const cell of frontier) {
    ...
    if (state.basin) {
      ...
      // Pockets are small — often four to eight cells — so cohesion alone barely
      // separates the shapes: nearly every candidate touches the region at one
      // cell. Whether a candidate reaches into a row or column the pocket does
      // not yet occupy is the signal that still carries at that size, and it is
      // what makes a blocky pocket read as a block and a snaking one as a line.
      const row = (cell / size) | 0
      const fresh =
        ((state.rowMask >>> row) & 1 ? 0 : 1) +
        ((state.colMask >>> (cell - row * size)) & 1 ? 0 : 1)
      weight *= POCKET_LINE_REACH[shape][fresh] ?? 1
...
/** The up-to-four 4-adjacent neighbours of a cell, written into `out`. */
const orthogonalNeighbours = (size: number, cell: number, out: number[]): number => {
...
export const regionsAreWellFormed = (size: number, regions: RegionMap): boolean => {
  ...
  for (let id = 0; id < size; id++) {
    ...
    while (stack.length > 0) {
      ...
      reached++
      const count = orthogonalNeighbours(size, cell, scratch)
